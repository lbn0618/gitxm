...
<form method="post" id="regForm">
    <div>
        ...
        <div class="tips" id="usernameTips">用户名不能为空</div>
    </div>
    <div>
        <label>密码</label>
        <input type="password" name="password" id="password" required>
        <div class="tips" id="pwdTips">密码不能少于6位</div>
    </div>
    <div>
        <label>邮箱</label>
        <input type="text" name="email" id="email" required>
        <div class="tips" id="emailTips">邮箱格式不正确</div>
    </div>
...
<script>
    ...
    username.addEventListener('blur', function() {
        ...
        const tips = document.getElementById('usernameTips');
        if(val === '') {
    ...
    email.addEventListener('blur', function() {
        const val = this.value.trim();
        const tips = document.getElementById('emailTips');
        const reg = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if(!reg.test(val) && val !== '') {
            tips.style.display = 'block';
        } else {
